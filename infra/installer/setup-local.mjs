...
async function writeLocalEnv(envMap) {
  ...
  // Preserve user-configured optional vars across re-runs (e.g. Google OAuth credentials).
...
# Google Calendar credentials are preserved across re-runs — edit them directly.
...
# Docker Compose reads this file to resolve \${SUPABASE_URL} and \${SUPABASE_ANON_KEY}
# in the atlas-web-local service. Uses localhost so the browser can reach Supabase.
