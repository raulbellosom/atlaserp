...
2. Existen dos implementaciones independientes del patrón "modal que se convierte en bottom-sheet en mobile": `Dialog.jsx` y `Sheet.jsx` (`packages/ui/src/components/`). Ambas ya comparten el hook `useDragToDismiss`, pero cada una calcula por separado su propio padding y su propio margen para el "handle" (la barra de arrastre superior): `Dialog.jsx` decide mobile-vs-desktop por CSS puro (breakpoints `md:`) mientras que `Sheet.jsx` usa un hook `useIsMobile` en JS (necesario porque además debe forzar `side="bottom"` en mobile aunque se pida `left`/`right`/`top`). Los valores de padding/handle no vienen de una fuente común: hoy dan un resultado numéricamente parecido, pero al no compartir código pueden desalinearse en cualquier cambio futuro, y visualmente ya se perciben distintas entre pantallas que usan cada una. Además, el nuevo selector de fecha/hora universal (ver Problema 1) también necesita decidir en JS si abrirse como `Popover` o como `Sheet`, así que sin extraer `useIsMobile` a un hook compartido se crearía una tercera copia.
