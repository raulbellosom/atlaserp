...
2. Riesgo: las credenciales S3-compatibles del contenedor de Supabase Storage no están documentadas en este repo (viven en `/opt/supabase-atlaserp/supabase/docker/.env` del VPS, fuera del repo). Mitigación: el plan de implementación debe incluir un paso explícito de verificar/generar esas credenciales en el VPS antes de configurar el `SegmentedFileOutput` de Egress.
