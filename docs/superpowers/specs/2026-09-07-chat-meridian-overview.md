...
| **2** | `2026-09-07-chat-meridian-spec-2-panel-design.md` — **Plan A (API) shipped 2026-09-07; Plan B (UI) pendiente** | Panel privado por usuario (`Sheet`), hilos `chat_meridian_thread/message`, contexto = conversación anfitriona + `focusMessageId`, `surface='panel'`. Motor `handlePanelMessage` + 3 endpoints + SDK listos y live-verificados | Spec 1 + Spec 4 |
