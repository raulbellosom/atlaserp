...
6. Structural protection for the default "Member" role, found during Task 4 code review: unlike Owner, Member is not `is_system`, so a sufficiently-ranked Admin/Moderator can rename or delete it via `updateRole`/`deleteRole`. Every role-deletion fallback and every new member/joiner resolves `role_id` by looking up `name = 'Member'` — if that role is renamed or deleted, they silently get `role_id = NULL` (no permissions, no error surfaced). Consider an `is_default` flag instead of relying on the name string once Phase B's role-management UI exists to make this reachable by a real user.
7. Row-level locking for `isLastOwner`'s check-then-act guard, found during Task 4 code review: two concurrent "demote the last Owner" requests could both read `count > 1` before either commits, both pass, leaving zero Owners. Not exploitable until Task 8 exposes this over HTTP — address with a `SELECT ... FOR UPDATE` inside a transaction, or an atomic conditional `UPDATE`, before or during Task 8.
