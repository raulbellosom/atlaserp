...
- **Message action menu** (`MessageActions` in `ChatMessageBubble.jsx`): a new "Responder en hilo" item (icon: `MessageSquare` or `Reply` from `lucide-react` — pick whichever isn't already used for something else in this file's imports), visible only for `channel`/`group` conversations, on any non-deleted message that is NOT itself already a thread reply (i.e. `message.thread_root_id` is falsy — a reply's own action menu never offers to start a nested reply; per Non-goal 1, clicking it on a reply would just have to redirect anyway, so hiding it entirely is simpler and avoids implying nesting exists).
- **Reply-count indicator**: when `message.thread_reply_count > 0` (see Section 14's casing note — this is a raw, unaliased top-level SQL column, snake_case, NOT `threadReplyCount`), render a small pill/button below the message body (same visual tier as the reaction-pill row Phase D added, positioned after it) reading "💬 N respuestas · hace Xm" (relative time from `message.thread_last_reply_at`, reusing `formatMessageTime`/existing relative-time helpers already in this module). Clicking it opens `ThreadPanel` for that message.
...
Response: unchanged shape (`{ data: Message }`), with the new `thread_root_id`/`thread_reply_count`/`thread_last_reply_at` fields populated per Section 14.
...
`chatSendMessageSchema` (existing, in `packages/validators/src/chat.js`) gains one new optional REQUEST field: `threadRootId: z.string().uuid().optional()` — camelCase, matching every sibling field already in this schema (`messageType`, `attachmentIds`, ...), since Zod-validated request bodies in this module are consistently camelCase.

No new schema for the thread-read endpoint (GET, no body).

**Casing pitfall — read before implementing (found live in this same phase's own prior task, Phase D)**: the RESPONSE shape is a different convention from the request. `getMessageFull`/`listMessages`' SQL selects top-level `chat_messages` columns unaliased (`m.thread_root_id`, not `m.thread_root_id AS "threadRootId"`) — Prisma's `$queryRaw` returns raw column names verbatim, so every top-level `Message` field in an API response is genuinely snake_case (`id`, `conversation_id`, `sender_user_id`, `created_at`, `deleted_at`, `pinned_at`, ...). Only *nested* JSON built via `json_build_object(...)` (the `sender`, `members`, `attachments`, `reactions` sub-objects) gets camelCase, because those explicitly name each JSON key in the query itself. Phase D's frontend implementation read `item.pinnedAt` (camelCase) against this exact snake_case contract, right next to correctly-cased sibling reads of `item.deleted_at`/`item.sender_user_id` in the same function — `isPinned` was silently always `false` in production until caught and fixed in this session (commit `7aea017`). Response shape addition (not a validator, but documented here since it's a contract both sides share): every `Message` object gains `thread_root_id` (string | null), `thread_reply_count` (number, always present, 0 by default), `thread_last_reply_at` (string | null, ISO timestamp) — all three snake_case, matching every other top-level message field. Do not alias them to camelCase in the SQL "for consistency with the nested objects" — that would only fix these three fields while leaving every pre-existing top-level field (and any future one) still snake_case, making the inconsistency worse, not better. Plan B must explicitly instruct the frontend implementer to use `message.thread_root_id`/`message.thread_reply_count`/`message.thread_last_reply_at` (snake_case) everywhere a `Message` object's own fields are read, while `threadRootId` (camelCase) is correct ONLY in the `sendMessage`/SDK REQUEST payload.
...
4. Given a user attempts to reply to a message that is itself already a thread reply, when the send completes, then the new message's `thread_root_id` is the ORIGINAL root's id, not the reply's id (auto-flatten, verified server-side).
5. Given a reply is deleted, when the root message's `thread_reply_count` is next read, then it has decremented by 1.
