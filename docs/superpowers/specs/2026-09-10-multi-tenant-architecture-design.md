...
Fixed both:
- `POST /companies` (system-admin only — reuses the already-seeded `company.profile.create` permission plus an explicit `tenant.isSystemAdmin` check, same shape as the existing `PATCH /companies/:companyId/modules` gate) creates a new `Company` + a `Membership` making the REQUESTING admin its first `atlas.admin` immediately (no separate Supabase Auth user, unlike `/setup/initialize`) + a default `BrandingConfig`. Slug collisions get a numeric suffix.
...
- `/user/me` gains `isSystemAdmin` (distinct from the existing `isAdmin`, which is company-admin-OR-system-admin) so the frontend can gate the "create company" affordance to true platform admins only.
- `CompanySwitcher.jsx` gains a "Crear empresa" option (system-admin only, via a new `CreateCompanyDialog.jsx`) that creates the company, waits for it to appear in the refetched membership list, then switches into it automatically.
