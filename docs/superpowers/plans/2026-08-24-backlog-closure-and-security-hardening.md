...
- [ ] Wire guest/seat management UI in the classic terminal (`PosTerminalScreen`): add a control to set `guestCount` on order creation and assign lines to `guestSeatId`. Backend + `SplitBillDialog` already support this — today only the mobile Comandero flow and reservations (`partySize`) reach it. Largest remaining item, not started.
