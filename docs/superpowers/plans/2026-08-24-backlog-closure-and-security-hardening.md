...
- [x] `SplitBillDialog.handleChargeSeat`: no longer silently no-ops when no payment method is configured — button disables with a `title` hint, and an `Alert` banner explains why. Verified: 2026-08-24 (`pnpm --filter @atlas/desktop build:web` clean).
- [x] Mis-mesas filter mode: backend (`pos-floor-service.js` `getFloorWithLayout` + `getActiveMap`) no longer excludes non-mine tables from the query — every table returns with its real status, annotated `isMine` only when the filter is active. `FloorOperationalCanvas.jsx` and `TableMap.jsx` dim (45% opacity) non-mine tables instead of ghosting them to a fake "Disponible". Verified: 2026-08-24 (`node --test apps/api/src/routes/pos/__tests__/*.test.js` → 62/62 incl. 2 new regression tests; desktop build clean).
- [x] Cerrar Caja dialog: "Efectivo esperado" preview now calls a new read-only `GET /pos/sessions/:id/expected-cash` endpoint (`computeExpectedCashAmount` extracted and shared with `closeSession`, which persists the identical value) instead of reading `session.expectedCashAmount` (a field only ever populated by `closeSession` itself, hence always $0.00 for an open session). Verified: 2026-08-24 (new regression test asserts preview and eventual close land on the exact same number; 61/61 → 65/65 POS tests pass; desktop build clean).
- [x] Resumed order on a table whose status had drifted back to AVAILABLE now re-occupies it: new `POST /pos/orders/:id/claim` endpoint (`claimOrder` service function, mirrors the `OCCUPIED + waiterId` logic `createOrder` already ran at creation time) wired into both `PosTablesScreen.navigateToOrder` (desktop terminal) and `ComandaScreen` (mobile Comandero) on resume. Idempotent — safe to call even when the table is already correct. Verified: 2026-08-24 (3 new regression tests: re-occupies drifted table, rejects non-editable orders, idempotent double-call; desktop build clean).
- [x] `helperText` prop leak to the DOM: was never a supported `TextField` prop (the component destructures `hint`, not `helperText` — anything else falls straight through to the native `<input>`). Fixed both occurrences (`PosSettingsScreen.jsx` tax field, `PosModifiersTab.jsx` price-delta field) by using the correct `hint` prop. Verified: 2026-08-24 (`grep -rn helperText apps/desktop/src` → no matches remain in atlas.pos; desktop build clean).
- [x] Wire guest/seat management UI in the classic terminal (`PosTerminalScreen`). Turned out smaller than expected once investigated: `SeatChips` (already built for Comandero) and `LineEditSheet`'s seat selector (already built, just never received a `guests` prop) were both fully reusable, generic, and already exercised end-to-end by the mobile flow — no new interaction design needed, just wiring. Delivered:
  - `OrderPanel.jsx`: guest count + "Agregar" button in the header (dine-in orders with a table only); each line shows its assigned comensal; `LineEditSheet` now receives `guests` so its existing seat selector renders — this alone unlocks reassigning any line to a seat.
  - `PosTerminalScreen.jsx`: `SeatChips` above the product grid (desktop + mobile) to pick the active seat before tapping products, mirroring Comandero exactly; new lines are stamped with the active seat; the pending-line merge-by-product logic now also matches on seat so two comensales ordering the same item don't collapse into one shared line.
  - No backend changes needed — `addOrderLine`/`updateOrderLine` already accepted `guestSeatId` (proven by Comandero); this was purely a frontend wiring gap.
  - Deliberately NOT done: a guest-count prompt at order creation. Comandero doesn't do this either (guests are added on-demand via "+"), so matching that established pattern was the more consistent choice than inventing a new one.
  Verified: 2026-08-24 (`pnpm --filter @atlas/desktop build:web` clean; backend untouched, 65/65 POS tests still pass since no service/route changed).
