...
- [ ] Cosmetic F3: el preview "Efectivo esperado" del diálogo Cerrar caja muestra $0.00 (no consulta el esperado real; el cálculo server-side sí es correcto)
...
- [ ] F4 pending: fallback `defaultStationId` en send-to-kitchen + UI/API de asignación producto↔estación (`posProductConfig` hoy no tiene gestión) + join `productName` en líneas del tablero KDS
...
- [ ] Cosmetic (pre-existing, commit `a45a256`): `helperText` prop leaks to DOM in PosSettingsScreen tax field → React warning in console
...
- [ ] Wire guest/seat management UI in the terminal: no control exists to set `guestCount` on order creation nor to assign lines to `guestSeatId`, so per-seat split bill is unreachable from the UI (backend + `SplitBillDialog` are ready; only reservations set `guestCount` via `partySize`) — scheduled as rework F2
