...
**PENDING — MANUAL ACTION REQUIRED (2026-09-13):** the user explicitly requested a full reset of the test database and object storage. Both actions were blocked by the Claude Code auto-mode safety classifier (a platform-level guard, separate from and in addition to Prisma's own AI-agent consent guard) — they must be run by a human directly in a terminal, not by an agent:

1. Database reset (wipes all data, reseeds with the `runly.*` catalog from this session's work):
   ```powershell
   cd D:\RacoonDevs\atlaserp-v2
   $env:PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION = "<literal text of your consent message>"
   pnpm db:reset
   ```
2. Storage cleanup (empties all objects — not the buckets themselves — in `module-bundles` (6 objects), `atlas-website` (412), `atlas-notes` (29), `atlas-chat` (92), `atlas-files` (131), counted 2026-09-13): run `node _tmp-clean-storage.mjs` from the repo root (temporary, untracked script left in place for this purpose; delete it after running).

Neither has been executed as of this writing. The rest of the clean-install cutover (canonical `runly.*` catalog, seed script, branding) is code-complete and waiting on these two manual steps before an actual empty-database acceptance pass can happen.
