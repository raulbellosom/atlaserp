...
export function createWopiTokens({ secret, tokenSeconds, now = Date.now }) {
  async function issue(claims) {
    ...
    return { accessToken: await sign({ ...claims, iat, exp, iss: 'runly-office', aud: 'runly-wopi' }, secret, 'HS256'), expiresAt: exp * 1000 };
  }
  async function validate(token, fileId) {
    try {
      ...
      if (claims.iss !== 'runly-office' || claims.aud !== 'runly-wopi' || claims.fileId !== fileId || !['view', 'edit'].includes(claims.mode) || !claims.authUserId || !claims.companyId || !claims.profileId || claims.exp * 1000 <= now() || claims.exp - claims.iat > tokenSeconds) throw new Error();
      return claims;
