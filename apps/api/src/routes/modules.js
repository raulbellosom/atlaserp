...
function rewriteBundleImportsForViteRuntime(source, webOrigin) {
  ...
  const cjsSpecRx = '(react|react-dom|react\\/jsx-runtime|react\\/jsx-dev-runtime)'

  rewritten = rewritten.replace(
    new RegExp(`import\\s+\\*\\s+as\\s+([A-Za-z_$][\\w$]*)\\s*from\\s*["']${cjsSpecRx}["'];?`, 'g'),
  ...
  rewritten = rewritten.replace(
    new RegExp(`import\\s+((?!__atlas_cjs_external_)[A-Za-z_$][\\w$]*)\\s*from\\s*["']${cjsSpecRx}["'];?`, 'g'),
    (_match, localName, specifier) => {
  ...
  rewritten = rewritten.replace(
    new RegExp(`import\\s*\\{([^}]+)\\}\\s*from\\s*["']${cjsSpecRx}["'];?`, 'g'),
  ...
  for (const specifier of MODULE_BUNDLE_EXTERNALS) {
    ...
    const importRx = new RegExp(`(\\bimport\\s*\\(\\s*["'])${escaped}(["']\\s*\\))`, 'g')
...
export function createModulesRouter({ prisma, authMiddleware, requirePermission, routeLoader = null, bundlerSvc = null }) {
  ...
  app.get('/:key/bundle.js', async (c) => {
    const key = c.req.param('key')
    const webOrigin = normalizeWebOrigin(c.req.query('web_origin') ?? c.req.header('origin') ?? null)
