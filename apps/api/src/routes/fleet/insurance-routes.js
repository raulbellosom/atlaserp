...
export function createInsuranceRouter({ prisma, requirePermission, moduleContext }) {
  ...
  app.get('/fleet/insurance', requirePermission('fleet.insurance.read'), async (c) => {
    try {
      ...
      return handleRouteError(c, err, { fallbackError: 'No se pudieron listar las polizas de seguro.', route: '/fleet/insurance', moduleKey, operation: 'listPolicies' })
    }
  })
  ...
  app.post('/fleet/insurance', requirePermission('fleet.insurance.create'), async (c) => {
    try {
      ...
      const result = await service.createPolicy({ companyId, actorId, data: parsed.data })
      return c.json({ data: result }, 201)
    } catch (err) {
      return handleRouteError(c, err, { fallbackError: 'No se pudo crear la poliza de seguro.', route: '/fleet/insurance', moduleKey, operation: 'createPolicy' })
    }
  })
  ...
  app.get('/fleet/insurance/:id', requirePermission('fleet.insurance.read'), async (c) => {
    try {
      const companyId = getCompanyIdFromContext(c)
      const result = await service.getPolicy({ companyId, id: c.req.param('id') })
      return c.json({ data: result })
    } catch (err) {
      return handleRouteError(c, err, { fallbackError: 'No se pudo obtener la poliza de seguro.', route: '/fleet/insurance/:id', moduleKey, operation: 'getPolicy' })
    }
  })
  ...
  app.patch('/fleet/insurance/:id', requirePermission('fleet.insurance.update'), async (c) => {
    try {
      ...
      if (!parsed.success) return c.json({ error: getValidationErrorMessage(parsed.error) }, 400)
      const result = await service.updatePolicy({ companyId, actorId, id: c.req.param('id'), data: parsed.data })
      return c.json({ data: result })
    } catch (err) {
      return handleRouteError(c, err, { fallbackError: 'No se pudo actualizar la poliza de seguro.', route: '/fleet/insurance/:id', moduleKey, operation: 'updatePolicy' })
    }
  })
  ...
  app.patch('/fleet/insurance/:id/enabled', requirePermission('fleet.insurance.delete'), async (c) => {
    try {
      ...
      if (parsed.data.enabled === true) {
        return c.json({ error: 'No se puede reactivar una poliza desactivada.' }, 400)
      }
      const result = await service.disablePolicy({ companyId, actorId, id: c.req.param('id') })
      return c.json({ data: result })
    } catch (err) {
      return handleRouteError(c, err, { fallbackError: 'No se pudo actualizar el estado de la poliza.', route: '/fleet/insurance/:id/enabled', moduleKey, operation: 'disablePolicy' })
    }
  })
  ...
  app.get('/fleet/vehicles/:vehicleId/insurance', requirePermission('fleet.vehicles.read'), async (c) => {
    try {
      ...
      const result = await service.listVehiclePolicies({ companyId, vehicleId })
      return c.json(result)
    } catch (err) {
