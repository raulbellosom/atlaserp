...
class ScreenSharePlugin(private val activity: Activity) : Plugin(activity) {
  ...
  fun start(invoke: Invoke) {
    if (!activity.hasWindowFocus()) { invoke.reject("Abre Atlas para compartir pantalla."); return }
    ...
    operation = scope.launch {
      ...
      try {
        withTimeout(25_000) {
          ...
          if (Build.VERSION.SDK_INT >= 26) manager.createNotificationChannel(NotificationChannel("atlas-screen-v1", "Pantalla compartida", NotificationManager.IMPORTANCE_LOW))
          val notification = NotificationCompat.Builder(activity, "atlas-screen-v1")
            .setSmallIcon(R.mipmap.ic_launcher_monochrome)
            .setContentTitle("Compartiendo pantalla")
            .setContentText("Puedes detener la transmisión desde Atlas o el control de Android.")
