...
export function FinanceAp({ token }) {
  ...
  const apDocumentsQuery = useQuery({
    ...
    queryFn: () =>
      atlas.finance.listDocuments(token, { direction: "AP", limit: 200 }),
  ...
  const apFilteredDocuments = useMemo(() => {
    return apDocuments.filter((doc) => {
      ...
      const statusMatch =
        apStatusFilter === "all" ? true : status === apStatusFilter;
  ...
  const pageMeta = SECTION_META.ap;
