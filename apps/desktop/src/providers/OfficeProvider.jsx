...
export function OfficeProvider({ children }) {
  ...
  const value = useMemo(() => ({ enabled: Boolean(canRead && status.data?.data?.enabled), available: Boolean(status.data?.data?.available), canEdit, open }), [canRead, canEdit, status.data, open]);
