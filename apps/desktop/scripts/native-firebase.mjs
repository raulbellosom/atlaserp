...
export function prepareAndroidFirebase(repository, androidApp, environment = process.env) {
  ...
  try {
    ...
    throw new Error('Cannot read Android Firebase configuration; check ATLAS_ANDROID_GOOGLE_SERVICES_JSON')
  }
  ...
  if (!config?.project_info?.project_id || !config.project_info.project_number
    || !config.client?.some((client) => client.client_info?.android_client_info?.package_name === 'com.racoondevs.atlaserp'
      && client.client_info?.mobilesdk_app_id && client.api_key?.some((key) => key.current_key))) {
    throw new Error('Firebase Android configuration must contain the app com.racoondevs.atlaserp')
  }
