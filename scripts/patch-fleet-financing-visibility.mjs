...
async function main() {
  ...
  if (Array.isArray(schema.sections)) {
    schema.sections.forEach((s) => {
      console.log(`  Section "${s.label}": ${s.fields?.map((f) => f.field).join(', ')}`)
    })
  }
  ...
  if (patched === 0) {
    ...
    console.log('Make sure the Financiamiento section exists in the form blueprint.')
    process.exit(0)
  }
  ...
  console.log('The financing fields will now be hidden when "Vehiculo financiado" is OFF.')
}
