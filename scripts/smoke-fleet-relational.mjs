...
const baseUrl = (process.env.RUNLY_API_URL || "http://localhost:4010").replace(
  /\/+$/,
  "",
);
const token = (process.env.RUNLY_TOKEN || "").trim();
...
async function main() {
  ...
  const tokenWithoutFleet = (process.env.RUNLY_TOKEN_NO_FLEET || "").trim();
  ...
  const tokenOtherCompany = (process.env.RUNLY_TOKEN_OTHER_COMPANY || "").trim();
